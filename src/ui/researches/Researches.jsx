...
export default function Researches({ data }) {
    return (
        <div dir="rtl" className="mt-8">
            {data.length ? <div style={{ rowGap: "32px", columnGap: "24px", display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(350px,1fr))" }}> {data.map((e) => <Card key={e.id} data={e} />)}</div> : <p>لا توجد نتيجة!!</p>}
        </div>
    )
}
